...
In ML Core, we represent data using Eigen matrices and vectors.
...
This means ML Core follows a row-major conceptual convention:
...
## 4. Why Shape Validation Matters
...
These helpers are not meant to replace Eigen.

They exist to make ML-specific assumptions explicit and to provide readable error messages.
# Math Foundations

This document contains the mathematical foundations used throughout ML Core.

ML Core uses Eigen for linear algebra, but the project still defines its own mathematical conventions so that all models, losses, metrics, and optimization routines use the same interpretation of vectors and matrices.
...
The first validation functions are:
...
This first math foundation step is intentionally small but important.
...
The purpose of this step is to make sure every future ML component starts from a consistent mathematical foundation.

---

## 7. Matrix Multiplication and Geometric Interpretation
...
If two vectors point in a similar direction, their dot product is large and positive.  
